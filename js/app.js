...
function containsProfanityOrNonsense(text){
  ...
  const badWords = ['sperm','jhonny','johnny','sins','sex','porn','fuck','shit','bitch','ass','damn','dick','cock','pussy','bastard','chor market','bloody','randi','lund','choot'];
  for(let word of badWords){ if(lower.includes(word)) return true; }
  if(/(.)\\1{3,}/.test(lower)) return true;
  const words = clean.split(/\s+/).filter(w => w.length > 1);
  if(words.length < 2) return true;
  for(let w of words){ if(w.length > 5 && !/[aeiouy]/i.test(w)) return true; }
  return false;
}
...
async function sendPersonalNotification(usernameOrId, title, message){
  ...
  list.unshift({ id: uid(), title, message, time: Date.now(), read: false });
...
async function checkAndPublishAutoNotice(){
  ...
  }catch(err){ console.error('Auto notice check failed', err); }
}
...
function renderAuthScreen(mode, errorMsg){
  ...
  if(mode==='login'){
    ...
    document.getElementById('reSubmit').onclick = async ()=>{
      ...
      if(!collegeEmail.includes('@')){ renderAuthScreen('register', 'Enter a valid college email address.'); return; }
      ...
      try{
        ...
        card.innerHTML = `
          ...
          <p class="sub">Your account has been registered with <strong>${esc(collegeEmail)}</strong> and is awaiting review by your Lab In-Charge or Owner.</p>
          <div style="background:var(--paper);border:1px solid var(--amber);padding:14px;border-radius:8px;margin:16px 0;font-size:13.5px;color:var(--ink);">
            Status: <strong style="color:var(--amber);">PENDING APPROVAL</strong><br/>
            <span style="font-size:12px;color:var(--ink-soft);margin-top:4px;display:block;">You will be able to sign in once approved. Ask your Lab In-Charge for the college code.</span>
          </div>
...
function showToast(msg, type='info'){
  ...
  toast.style.cssText = `background:${c.bg};color:${c.fg};padding:10px 14px;border-radius:8px;font-size:13px;font-family:'IBM Plex Sans',sans-serif;box-shadow:0 6px 16px rgba(0,0,0,0.25);pointer-events:all;`;
...
async function renderInventory(){
  ...
  const drawList = ()=>{
    ...
    list.querySelectorAll('[data-edit-details]').forEach(b=> b.onclick = ()=>{ editingDetails.add(b.dataset.editDetails); drawList(); });
    list.querySelectorAll('[data-cancel-edit]').forEach(b=> b.onclick = ()=>{ editingDetails.delete(b.dataset.cancelEdit); drawList(); });
...
async function renderCheckout(){
  ...
  equipment.forEach(eq => {
    ...
    if(!hasPendingOrActive && eq.availableQty < eq.totalQty){ eq.availableQty = eq.totalQty; stockUpdated = true; }
  });
  if(stockUpdated){ await saveList(KEYS.equipment, equipment, true); }
  const main = document.getElementById('main');
  ...
  document.getElementById('coSubmit').onclick = async ()=>{
    ...
    if(containsProfanityOrNonsense(purpose)){ showToast('Invalid description. Please provide a meaningful purpose.', 'error'); purposeInput.focus(); return; }
    const eq = equipment.find(x=>x.id===eqId);
    ...
    <div class="grid grid-2" style="align-items:start;">
      <div class="panel">
        ...
        <div id="camWrap" style="position:relative;background:#050B14;border-radius:14px;overflow:hidden;aspect-ratio:1/1;max-width:420px;margin:0 auto;display:flex;align-items:center;justify-content:center;box-shadow:0 12px 36px rgba(0,0,0,0.3);border:2px solid var(--grid);">
          <video id="scanVideo" muted autoplay playsinline disablePictureInPicture style="width:100%;height:100%;object-fit:cover;display:none;"></video>
...
async function lookupAndShow(tagOrId){
  ...
  if(!resultEl) return;
  if(!eq){ resultEl.innerHTML = `<div class="empty">No equipment matches "${esc(tagOrId)}" — check the tag code and try again.</div>`; return; }
  ...
  resultEl.innerHTML = `
    <div class="panel">
      ...
      ${eq.description ? `<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:0.03em;margin-bottom:6px;">Description</div><div style="font-size:13.5px;line-height:1.5;white-space:pre-wrap;">${esc(eq.description)}</div></div>` : ''}
      ${eq.usageNotes ? `<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:0.03em;margin-bottom:6px;">How to use</div><div style="font-size:13.5px;line-height:1.5;white-space:pre-wrap;background:var(--paper);border:1px solid var(--grid);border-radius:6px;padding:10px 12px;">${esc(eq.usageNotes)}</div></div>` : ''}
      ${eq.videoUrl ? `<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:0.03em;margin-bottom:6px;">Video</div>${videoEmbedHtml(eq.videoUrl)}</div>` : ''}
      ${eq.photoUrls ? `<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:0.03em;margin-bottom:6px;">Photos</div>${photosGalleryHtml(eq.photoUrls)}</div>` : ''}
      ${eq.extraLinks ? `<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:0.03em;margin-bottom:6px;">Extra Links</div>${linksListHtml(eq.extraLinks)}</div>` : ''}
      ${activeCheckout ? `<div style="margin-bottom:14px;"><div style="font-size:12px;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:0.03em;margin-bottom:6px;">Currently checked out</div><div style="font-size:13.5px;">${esc(activeCheckout.borrower)} · ×${activeCheckout.qty} · due ${fmtTime(activeCheckout.dueTime)}</div></div>` : `<div style="margin-bottom:14px;color:var(--ink-soft);font-size:13px;">Not currently checked out.</div>`}
      ${openIssue ? `<div style="margin-bottom:14px;"><span class="badge badge-rust">Open maintenance report</span><div style="font-size:13.5px;margin-top:6px;">${esc(openIssue.issue)}</div></div>` : ''}
      <div>
...
async function renderUsers(){
  ...
  }catch(e){ document.getElementById('usersBody').innerHTML = `<div class="empty">Could not load users.</div>`; return; }
  const remoteApprovals = await storageGet(KEYS.approvedUsersMap, true) || {};
  users.forEach(u => {

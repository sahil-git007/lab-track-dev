...
function renderSidebar(){
  const sb = document.getElementById('sidebar');
  const navContent = buildNav().map(g=>`
  ...
  sb.innerHTML = `
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 14px 16px 6px 16px; border-bottom: 1px solid var(--grid); margin-bottom: 8px;">
      <span style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; color: var(--ink-soft); letter-spacing: 0.05em;">Navigation</span>
      <button id="sidebarCloseBtn" style="background: none; border: none; font-size: 1.2rem; cursor: pointer; color: var(--ink); padding: 0 4px;" title="Close Menu">×</button>
    </div>
